...
export async function writeGeneratedFiles(options: FileWritingOptions): Promise<void> {
  const { outputDir, clientClass, routeGroupClasses, clientName, typesFile } = options;
  ...
  // Write index files
  await writeIndexFiles(outputDir, clientName, Object.keys(routeGroupClasses), !!typesFile);
}
...
async function createDirectoryStructure(outputDir: string): Promise<void> {
  const directories = [outputDir, join(outputDir, 'routes'), join(outputDir, 'errors')];
...
async function writeIndexFiles(
  ...
  await writeMainIndexFile(outputDir, clientName, routeGroupNames, hasTypes);
...
async function writeMainIndexFile(
  ...
  const indexContent = generateMainIndexContent(clientName, routeGroupNames, hasTypes);
