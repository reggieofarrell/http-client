...
describe('HttpClient Idempotency', () => {
  ...
  describe('Idempotency Key Generation and Caching', () => {
    ...
    it('should generate different keys for two different FormData uploads to the same URL', async () => {
      // Regression test: the old signature-based cache stringified the request
      // body via JSON.stringify, which turns any FormData into "{}" regardless
      // of its contents - causing two different uploads to collide on the same
      // idempotency key. The generator no longer depends on the request body at
      // all, so this can't happen.
      client = new HttpClient({
        baseURL: 'https://api.example.com',
        idempotencyConfig: {
          enabled: true,
      ...
      formData1.append('file', 'contentA');
      const formData2 = new FormData();
      formData2.append('file', 'contentB');

      await client.post('/upload', formData1);
      await client.post('/upload', formData2);
    ...
    it('manual retries should reuse the same key only when idempotencyKey is passed explicitly', async () => {
      // Automatic keys are generated fresh per request() call by design - an
      // application-level retry (catching an error and calling client.post()
      // again) must pass its own idempotencyKey to guarantee reuse.
      client = new HttpClient({
        baseURL: 'https://api.example.com',
        idempotencyConfig: {
          enabled: true,
      ...
      const manualKey = 'app-managed-retry-key';
      await client.post('/test', { data: 'test' }, { idempotencyKey: manualKey });
      await client.post('/test', { data: 'test' }, { idempotencyKey: manualKey });

      const calls = mock.history.post;
      expect(calls?.[0]?.headers?.['Idempotency-Key']).toBe(manualKey);
      expect(calls?.[1]?.headers?.['Idempotency-Key']).toBe(manualKey);
    });
  });
